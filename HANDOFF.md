...
- CLI `foundry` with five subcommands: `demo` (nine-step story), `verify` (full evidence re-verification incl. independent recomputation of every paired analysis; exit 0/1), `lineage`, `replay` (re-executes a recorded mission and compares output digests; exit 0/1), `coverage` (event-coverage audit vs the 95% exit criterion; exit 0/1).
...
- Docs: `README.md`, `docs/ARCHITECTURE.md`, `docs/DECISIONS.md` (ADR-001..012), `docs/ROADMAP.md`, `research/protocols/STAGE1_PROTOCOL.md`, adapter README stubs under `adapters/`.
...
python -m pytest -q                          # expect: 452 passed; LangGraph conformance and the live OpenAI test skip without the extra / OPENAI_API_KEY
...
1. **GEPA/DSPy library adapters** in `adapters/optimizers/gepa_dspy/` (optional, when evolutionary search over candidates is wanted): same `ProposerLike` seam the OpenAI reflective proposer already fills; no promotion authority, no vault access, ever.
...
3. Also open (smaller): the Stage-1 trace UI (the last open 19.1 item; CLI verify/lineage/replay/coverage is the current surface).
