...
2. `docs/RESUME_GUIDE.md`: how to work in this repo effectively (the working loop, patterns, gotchas).
3. `docs/TECHNICAL_REPORT.md` is the authoritative spec; source docstrings cite the sections they implement.
...
- **Verify before claiming done.** Run `python -m pytest -q` (the full suite must pass; the current count is in `HANDOFF.md`) and `python -m ruff check src tests` (expect clean). A change is not done while either fails.
