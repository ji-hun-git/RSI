...
- Tool gateway (report 10.2/14.3/14.4, Stage-2): `foundry.tools.ToolGateway` = capability-bound tool calls (deny-by-default, non-transferable, revocation-aware via injectable authorizer), discovery!=authorization, `EgressPolicy` SSRF blocking (loopback/private/link-local/metadata + optional domain allowlist) before the tool runs, per-call ledger receipts (TOOL_AUTHORIZED/CALLED/RESULT/SIDE_EFFECT/DENIED with arg+response digests, latency, side-effect class), untrusted output, output-size cap, auth-checked idempotent replay, and `register_conformant` (side-effect-aware tool conformance: determinism only for pure tools; refuses lying/invalid-class/wrong-shape/crashing tools; ADR-018). Uses existing event types (no contract change). `tests/test_tools.py`, `tests/test_tool_conformance.py`; ADR-016/018. Open: DNS-pinning egress proxy, wire gateway into the worker/runtime call path.
- Docs: `README.md`, `docs/ARCHITECTURE.md`, `docs/DECISIONS.md` (ADR-001..018), `docs/ROADMAP.md`, `research/protocols/STAGE1_PROTOCOL.md`, adapter README stubs under `adapters/`.
...
python -m pytest -q                          # expect: 536 passed; LangGraph conformance and the live OpenAI test skip without the extra / OPENAI_API_KEY
...
1. **Stage-2 continuation**: route tool use through the `ToolGateway` from inside a worker (needs a tool-using-worker layering; real tool workers are impure, which the deterministic-fixture conformance suite would flag), serialize module admission across restarts, PydanticAI worker adapter + real MCP tool adapter behind the gateway (tool conformance suite done), and signed bundle supply chain (Cosign/SBOM). Also open: GEPA/DSPy library adapters behind the filled `ProposerLike` seam.
