...
- [~] Module registry with hot-swap tests: the registry, the report 17.3 shadow-execution replacement check (`check_replacement`), and runtime resolution (`ModuleResolvingRuntime` resolves a bundle's `module_refs` through the conformance-gated registry, so a mission runs only an admitted module; ADR-015/017) exist; the full 3-worker/2-tool-provider bar and a tool-provider conformance suite remain open
