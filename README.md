...
- `tools/` - the capability-bound tool gateway (report 10.2/14.3/14.4): every tool call (native or MCP) is terminated in one internal model - discovery separated from authorization, a scoped non-transferable capability token gates each call (deny-by-default), an SSRF/egress policy blocks internal targets before the tool runs (with optional resolve-and-pin DNS-rebinding defense), output is size-bounded and flagged untrusted, and a receipt (argument/response digests, capability, latency, side-effect class) is written to the ledger; a repeated side-effecting call is idempotent-on-key but still authorization-checked. `register_conformant` gates tool admission on a side-effect-aware conformance suite (a tool cannot lie about being pure, crash on valid input, or declare an invalid side-effect class); `ToolAugmentedRuntime` runs a `ContextualWorker` whose tool use flows through the gateway under a least-privilege per-mission capability, so a mission's tool calls are all authorized and receipted in one audit trail
...
python -m pytest                      # 565 tests (LangGraph conformance and the live OpenAI test skip without extra/key)
...
├── tests/                # 565 tests, including tests/test_e2e_replay.py (capstone)
