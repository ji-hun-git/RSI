...
| LangGraph (or other model-backed) runtime | Crash/replay conformance was validated model-free first (report 19.5 weeks 3-4) | `adapters/runtimes/langgraph/` behind `foundry.runtime.adapter.RuntimeAdapter` |
...
The adapter directories exist as empty seams with README stubs so future integrations land behind the frozen protocols instead of inside the control plane.
